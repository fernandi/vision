...
without loading the model and index on this machine (~3 GB of RAM).
    ...
    python dev_server.py --api http://localhost:8000 --native-ids

The deployed backend may predate `reference_ids` / `negative_ids` (searching
from an already-indexed image). Unless --native-ids is given, those ids are
converted here into uploaded images: the image is fetched server-side, where
museum CDNs don't enforce CORS.
"""
import argparse
import base64
import json
...
API_PREFIXES = ("/search", "/cluster-members", "/health", "/auth/", "/flag")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/search" and not self.native_ids:
...
def main():
    ...
    p.add_argument("--native-ids", action="store_true",
                   help="backend supports reference_ids/negative_ids: forward them untouched")
    args = p.parse_args()
    mimetypes.add_type("text/javascript", ".js")
    Handler.api = args.api.rstrip("/")
    Handler.native_ids = args.native_ids
